export const about = {
  path: "/en/about",
  seoTitle: "About | Yusuf Bayrak",
  seoDescription:
    "Professional background of Yusuf Bayrak — data-driven digital marketing, B2B lead generation and AI workflow automation.",
  heading: "About",
  paragraphs: [
    "I blend data-driven digital marketing strategy with a strong analytical lens. Performance marketing, B2B lead generation and digital infrastructure management are my core focus areas.",
    "The scope is not limited to ad management: from website builds to advanced automation workflows, a brand's digital ecosystem is engineered end-to-end. AI tools that cut operational load and improve budget efficiency sit at the center of the process.",
    "Alongside SaaS products like AdGusto, Brandog and Marka-MCP, I turn the systems I run and validate in-market into sustainable growth engines for brands.",
  ],
  approachDivider: "Approach",
  approach: [
    "Data-driven decisions come before intuition",
    "Reduce operational load with automation, unlock scale",
    "Strengthen the marketing stack with SaaS products",
    "Don't just plan — test it in-market and ship results",
  ],
  expertiseLabel: "Expertise",
  expertise: [
    "Performance Marketing",
    "B2B Lead Generation",
    "AI & Workflow Automation",
    "Web Development & Setup",
    "Growth Strategy",
    "SaaS Product Development",
  ],
  toolsLabel: "Tools",
  tools: ["Google Ads", "Meta Ads", "LinkedIn Ads", "AI APIs / LLMs", "n8n", "Cursor", "Lovable", "Web Analytics"],
  experienceLabel: "Experience",
  experience: [
    "Performance marketing & automation",
    "B2B lead generation systems",
    "SaaS product development",
  ],
};
